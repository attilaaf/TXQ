import * as supertest from 'supertest';
import { createExpress } from '../../../../src/bootstrap/express-factory';
let config = require(__dirname + '/../../../../src/cfg/index.ts').default;
let version = require(__dirname + '/../../../../src/api/v1/index.ts');
let url = config.baseurl + ':' + config.api.port;
let api;
const boot = async () => {
  api = supertest(await createExpress())
};
boot();

describe('channel', () => {

  test('post rawtx null channel 200', async (done) => {
 
    api
      .post(`${version.path}/tx`)
      .send({
        channel: null,
        set: {
          '3191c8f14fd1171d974f965963924966de49d15bad910a38e33dc44af14929e6': {
            rawtx: '',
            meta: {
              title: 'abc',
              content: 'content123',
              url: 'url',
              image: 'image',
              description: 'desc'
            },
            tags: []
          },
          '7895bfd75e0db3c0305d3edc098e3edbe326c451f155305f4c8f5fd76096161f': {
            rawtx: '',
          }
        }
      })
      .set('Accept', 'application/json')
      .expect('Content-Type', /json/)
      .expect(200)
      .end((err, res) => {
        expect(res.body.status).toBe(200);
        // So we process the tx
        setTimeout(function (e) {
          done();
        }, 0)
      });
  });

  test('post rawtx foobar channel 200', async (done) => {
    api = supertest(await createExpress());
    api
      .post(`${version.path}/tx`)
      .send({
        channel: 'foobar',
        set: {
          '3191c8f14fd1171d974f965963924966de49d15bad910a38e33dc44af14929e6': {
            rawtx: '',
            meta: {
              title: 'foobar abc',
              content: 'foobar content123',
              url: 'foobar url',
              image: 'foobar image',
              description: 'foobar desc'
            },
            tags: [ 'foobartag' ]
          }
        }
      })
      .set('Accept', 'application/json')
      .expect('Content-Type', /json/)
      .expect(200)
      .end((err, res) => {
        expect(res.body.status).toBe(200);
        // So we process the tx
        setTimeout(function (e) {
          done();
        }, 0);
      });
  });

  test('default channel 200', async (done) => {
    api = supertest(await createExpress());
    api
      .get(`${version.path}/channel`)
      .expect(200)
      .end((err, res) => {
        expect(res.body.status).toBe(200);
        expect(res.body.result.length > 0).toEqual(true);
        done();
      });
  });

  test('default channel 200 use id and limit', async (done) => {
    api = supertest(await createExpress());
    api
      .get(`${version.path}/channel?limit=1`)
      .expect(200)
      .end((err, res) => {
        expect(res.body.status).toBe(200);
        expect(res.body.result.length).toEqual(1);
        done();
      });
  });

  test('default channel 200 use id and limit 2 CURRENTS', async (done) => {
    api = supertest(await createExpress());
    const res = await api
      .get(`${version.path}/channel?limit=2`)
      .expect(200);
      //.end(async (err, res) => {
      expect(res.body.status).toBe(200);
      expect(res.body.result.length).toEqual(2);

    const firstId = res.body.result[0].id;
    const res2 = await api
      .get(`${version.path}/channel?limit=2&id=${firstId}`)
      .expect(200);

      expect(res2.body.status).toBe(200);
      expect(res2.body.result.length).toEqual(2);
      expect(res2.body.result[0].id !== firstId).toEqual(true);
      done();
  });

  test('Query with valid AccountContext', async (done) => {
    api = supertest(await createExpress());
    const res = await api
      .get(`${version.path}/channel`)
      .set({
        api_key:  'key1',
        project_id: 'default',
      })
      .expect(200);

    expect(res.body.status).toBe(200);

    done();
  });

  test('Query with invalid AccountContext', async (done) => {
    api = supertest(await createExpress());
    const res = await api
      .get(`${version.path}/channel`)
      .set({
        api_key:  'key1',
        project_id: 'project1bad',
      })
      .expect(403);

    expect(res.body.status).toBe(403);

    done();
  });

  test('Query with empty AccountContext success', async (done) => {
    api = supertest(await createExpress());
    const res = await api
      .get(`${version.path}/channel`)
      .set({
        api_key:  '',
        project_id: '',
      })
      .expect(200);

    expect(res.body.status).toBe(200);

    done();
  });

  test('Query with partial empty AccountContext fail', async (done) => {
    api = supertest(await createExpress());
    const res = await api
      .get(`${version.path}/channel`)
      .set({
        api_key:  '',
        project_id: 'project1',
      })
      .expect(403);

    expect(res.body.status).toBe(403);

    done();
  });

  test('Query with partial empty AccountContext success key not used default', async (done) => {
    api = supertest(await createExpress());
    const res2 = await api
      .get(`${version.path}/channel`)
      .set({
        api_key:  'key1',
        project_id: '',
      })
      .expect(200);

    expect(res2.body.status).toBe(200);

    done();
  });

});
